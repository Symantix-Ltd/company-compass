...
import sicData from '@/data/sic.json' assert { type: 'json' };
...
export default async function SearchPage({ params }: { params: Promise<Params> }) {
  ...
  const sicMap: Record<string, string> = sicData;
  const sicLabel = sicMap[sic_code];
  ...
  if (sic_code) {
    try {
      const res = await fetch(
        `${process.env.BASE_URL}/api/search/sic_code?q=${encodeURIComponent(sic_code)}`,
        { cache: 'no-store' }
      );
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 text-gray-900">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
        <aside className="order-3 lg:order-1 w-full lg:w-1/4 p-5">
          <h2 className="text-2xl">Company Insolvency</h2>
          <h3 className="text-pink-800 font-bold py-2">
            Notices published in{' '}
            <a className="italic" href="https://www.thegazette.co.uk/all-notices">
              The Gazette
            </a>{' '}
            on{' '}
        ...
        <main className="order-1 lg:order-2 flex-1 p-5">
          ...
          <h2 className="text-2xl text-pink-700 mb-4">
            <span className="font-bold">
              {sic_code} - {sicLabel ?? 'Unknown SIC Code'}
            </span>
          </h2>
        ...
        <aside className="order-2 lg:order-3 w-full lg:w-1/4 p-4">
