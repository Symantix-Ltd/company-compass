...
export async function generateMetadata(
    ...
    const title = `${data?.CompanyName} - Gazette Notices - Free business summary taken from official Companies House information. Registered as ${company_number}`;

    const description = `${data?.CompanyName} ${company_number}  - Gazette Notices - company located in ${data?.RegAddress_PostTown}, ${data?.RegAddress_PostCode}. Check company credentials including financials, industry, and contact information from Companies House and The Gazette - Company Compass UK`;
    return {
      title: title,
      description: description,
      author: "Company Compass",
      keywords: `${data?.CompanyName} ${company_number} ${data?.RegAddress_PostTown} ${data?.RegAddress_PostCode} Gazette Notices`,
      alternates: {
          canonical: `https://www.companycompass.co.uk/company/${company_number}/${company_name}/gazette-notices`,
        },
